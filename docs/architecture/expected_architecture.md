...
[Command WAL]                      [partially implemented: WAL storage + DTOs exist]
        |
        v
[Instrument Engine]                [partially implemented: NewOrder engine]
        |
...
| Command WAL | partially implemented | WAL subsystem exists; command DTO exists; NewOrder matcher integration is implemented. |
| Instrument Engine | partially implemented | NewOrder-only engine owns OrderBook and is wired through Command WAL -> Event WAL. |
| OrderBook | partially implemented | In-memory NewOrder FSM exists; Cancel/Replace are not implemented yet. |
| Continuous Matcher | partially implemented | NewOrder command application is implemented; Cancel/Replace and richer matching modes are not implemented yet. |
...
| Execution Event WAL | partially implemented | WAL subsystem exists; execution event DTO exists; NewOrder events are generated by the matcher pipeline. |
...
MATCHING SIDE
  Read committed commands              [partially implemented at WAL reader level]
      |
      v
  Apply to instrument state            [partially implemented: NewOrder InstrumentEngine]
      |
      v
  Emit execution events                [partially implemented: NewOrder event sequences]
...
```text
Instrument Engine:                    [partially implemented: NewOrder]
  - market phase
  - order book
