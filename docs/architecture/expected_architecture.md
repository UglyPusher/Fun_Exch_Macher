...
[Command WAL]                      [partially implemented: WAL storage + DTOs + matcher pipeline exist]
        |
        v
[Instrument Engine]                [partially implemented: NewOrder + CancelOrder engine]
        |
        +--> [OrderBook]           [partially implemented: NewOrder + CancelOrder FSM]
        +--> [Continuous Matcher]  [partially implemented: NewOrder + CancelOrder]
...
| Command WAL | partially implemented | WAL subsystem exists; command DTO exists; NewOrder and CancelOrder matcher integration is implemented. |
| Instrument Engine | partially implemented | NewOrder and CancelOrder engine owns OrderBook and is wired through Command WAL -> Event WAL. |
| OrderBook | partially implemented | In-memory NewOrder and CancelOrder FSM exists; Replace is not implemented yet. |
| Continuous Matcher | partially implemented | NewOrder and CancelOrder command application is implemented; Replace and richer matching modes are not implemented yet. |
...
| Execution Event WAL | partially implemented | WAL subsystem exists; execution event DTO exists; NewOrder and CancelOrder events are generated by the matcher pipeline. |
...
MATCHING SIDE
  Read committed commands              [partially implemented at WAL reader level]
      |
      v
  Apply to instrument state            [partially implemented: NewOrder + CancelOrder InstrumentEngine]
      |
      v
  Emit execution events                [partially implemented: NewOrder + CancelOrder event sequences]
...
```text
Instrument Engine:                    [partially implemented: NewOrder + CancelOrder]
  - market phase
  - order book
